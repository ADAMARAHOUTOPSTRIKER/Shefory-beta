...
- **Nom** : `shefory` · **Région** : `eu-west-3` (Paris, faible latence Maroc)
...
| `subscription_packs` | Catalogue Bronze / Argent / Or (prix en DH/MAD, poids classement, features). |
| `subscriptions` | Abonnement actif d'un chauffeur (1 actif max, lié à un moyen de paiement marocain). |
| `payments` | Miroir des transactions (montant DH, méthode : carte CMI / Orange Money / inwi money / Cash Plus, statut) — alimente l'admin. |
...
| Email | Rôle | Pack |
|---|---|---|
| `youssef@demo.shefory.app` | chauffeur | Or |
| `salma@demo.shefory.app` | chauffeur | Argent |
| `karim@demo.shefory.app` | chauffeur | Or |
| `fatima@demo.shefory.app` | chauffeur | Bronze |
| `mehdi@demo.shefory.app` | chauffeur | gratuit (non vérifié) |
